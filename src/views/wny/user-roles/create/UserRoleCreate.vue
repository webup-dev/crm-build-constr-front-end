  ...
  <div class="animated fadeIn">
    <b-row>
      <b-col md="6">
        <b-card>
          <div slot="header">
            <strong>Create User Roles</strong>
          </div>
          <b-form
    ...
    export default {
        ...
        methods: {
            ...
            userRolesCreatingSuccessful() {
                this.errors = false;
                this.error = false;
                this.flash('New User Roles created.', 'success', {timeout: VUE_APP_FLASH_TIMEOUT});

                this.$router.replace(this.$route.query.redirect || '/user-roles/index')
            },

            userRolesCreatingFailed(req) {
                this.errors = false;
                this.error = 'User Roles Creating failed! ' + req;
                console.log(req);
            },
        },
        mounted() {
