...
<script>
  ...
  import {addRequester, getOrganizations} from "../../../api/requesters";
  ...
  export default {
    ...
    methods: {
      ...
      requesterCreatingSuccessful() {
        this.errors = false;
        this.error = false;
        this.flash('New Requester is created.', 'success', {timeout: VUE_APP_FLASH_TIMEOUT});
      ...
      requesterCreatingFailed(req) {
        this.errors = false;
        this.error = 'Requester Creating failed! ' + req;
        console.log(req);
      },
      saveAndNew() {
        let dataPost = this.dataPost();
        addRequester(dataPost)
          .then(() => this.requesterStoringSuccessful())
          .catch((request) => this.requesterCreatingFailed(request));
      },
      requesterStoringSuccessful() {
        this.flash('New Requester is created.', 'success', {timeout: VUE_APP_FLASH_TIMEOUT});
        this.cancel();
        this.downloadData();
      },
      dataPost() {
        return {
          ...
          note: this.note,
          created_by_id: store.state.user.id
        };
      },
