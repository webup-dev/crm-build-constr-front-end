<template>
  <div className="animated">
    <b-card>
      ...
      <b-card-header>
        <i class="icon-menu mr-1"></i>Workflows

        <div class="card-header-actions"></div>
      </b-card-header>
      <b-card-body>
        <a href="/#/admin/workflow/create" class="btn btn-warning" style="float: right">Create Workflow</a>
...
<script>
  ...
  export default {
    ...
    data: function () {
      return {
        columns: ['id', 'name', 'organization_id', 'organization.name', 'workflow_type', 'stageSequence', 'description', 'actions'],
        data: [],
        message: '',
        success: false,
        res: [],
        options: {
          ...
          sortable: ['id', 'name', 'organization_id', 'organization.name', 'workflow_type'],
          filterable: ['id', 'name', 'organization_id', 'organization.name', 'workflow_type' ],
    ...
    methods: {
      ...
      downloadData() {
        getWorkflows()
          .then(response => {
            ...
            this.data = this.formatData(response.data.data);
      ...
      formatData(data) {
        data.forEach(function (item, index) {
          ...
          stages.sort((a,b) => {return (a.pivot.order > b.pivot.order) ? 1 : ((b.pivot.order > a.pivot.order) ? -1 : 0);} );
          let seq = '';
          stages.forEach(function (el) {
              seq = seq === '' ? el.name : seq + " -> " + el.name
          });
          item.stageSequence = seq;
        });
